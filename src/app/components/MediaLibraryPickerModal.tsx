...
export function MediaLibraryPickerModal({ open, onClose, onSelect }: Props) {
  ...
  const filteredMedia = useMemo(() => {
    const inFolder = mockMedia.filter((f) => f.folderId === currentFolderId);
    const q = searchQuery.trim().toLowerCase();
    if (!q) return inFolder;
    return inFolder.filter((f) => f.name.toLowerCase().includes(q));
  }, [currentFolderId, searchQuery]);

  const currentFolders = mockFolders.filter((f) => f.parentId === currentFolderId);

  const getBreadcrumbs = (): FolderItem[] => {
    const breadcrumbs: FolderItem[] = [];
    let folder: FolderItem | undefined = mockFolders.find(
      (f) => f.id === currentFolderId
    );
    while (folder) {
      breadcrumbs.unshift(folder);
      folder = mockFolders.find((f) => f.id === folder!.parentId);
    }
    return breadcrumbs;
  };
  ...
  const renderFolderTree = (parentId: string | null, level = 0) => {
    return mockFolders
      ...
      .map((folder) => {
        const hasChildren = mockFolders.some((x) => x.parentId === folder.id);
        const isExpanded = expandedFolders.has(folder.id);
        const isActive = currentFolderId === folder.id;
        const { count } = folderStats(folder.id, mockMedia);
  ...
  const rootCount = mockMedia.filter((f) => f.folderId === null).length;
              ...
              {currentFolders.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                  {currentFolders.map((folder) => {
                    const { count, bytes } = folderStats(folder.id, mockMedia);
                    return (
                      <button
                        key={folder.id}
                        type="button"
