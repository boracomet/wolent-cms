...
export function AnalyticsDashboard() {
  ...
  const loginsSeries = useMemo(() => {
    ...
    if (apiStats?.dailySeries?.length) {
      return apiStats.dailySeries.map((d: any) => ({
        dayKey: d.date,
        value: d.views,
        label: d.date,
      }));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-xl border border-zinc-800/80 bg-zinc-900/50 backdrop-blur-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-zinc-800/80 flex flex-wrap items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-zinc-200">{t("analytics.topPagesHeading")}</h2>
                ...
                {apiStats?.topPages?.length ? (
                  (apiStats.topPages as { url: string; views: number }[]).map((row, i) => (
                    <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
                      <td className="px-6 py-3 font-mono text-xs text-zinc-300 max-w-xs truncate" title={row.url}>{row.url}</td>
                      <td className="px-6 py-3 text-right text-zinc-400 tabular-nums">{row.views}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
